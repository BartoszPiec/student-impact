...
const categoryGradients: Record<string, string> = {
    ...
    tlumaczenia: "from-green-500 to-emerald-600",
...
function getGradient(category: string | null) {
    ...
    return categoryGradients[normalizeText(category)] || categoryGradients.default;
}
...
export default async function CustomizePackagePage(props: {
    ...
    const { data: pkg, error } = await supabase
        ...
        .select("id, title, category, type, is_system, student_id, form_schema, variants")
        .eq("id", params.id)
        .single();

    if (error) {
        redirect(`/app/company/packages/${params.id}`);
    }
    ...
    const selectedVariant = resolveSelectedPackageVariant(variants, searchParams.variant);
    const isSystemPackage = isSystemServicePackage(pkg);
    const displayTitle = pkg.id === LOGO_PACKAGE_ID ? "Projekt Logo" : pkg.title;
        ...
        <div style={fullBleedStyle} className="min-h-screen bg-slate-50/50 pb-20 font-sans">
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
                    ...
                    <div className="mb-2 text-white">
                        ...
                        <p className="text-xl md:text-2xl text-slate-300 leading-relaxed font-light">
                            Wypelnij formularz, aby dopasowac cel, format i zakres realizacji do Twoich wymagan.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 -mt-20 relative z-20">
                <CustomizePackageForm
                    ...
                    gradient={gradient}
                    formSchema={isSystemPackage ? formSchema : []}
