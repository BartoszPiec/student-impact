...
export function VariantPicker({ variants, baseDeliveryDays, gradient, packageId }: VariantPickerProps) {
    // Default to middle variant (M) if 3 variants, otherwise first
    const defaultIndex = variants.length === 3 ? 1 : 0;
    ...
    useEffect(() => {
        const handleSelectVariant = (e: Event) => {
            ...
            const index = variants.findIndex(v => v.name === customEvent.detail);
            if (index !== -1) {
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-2xl shadow-indigo-500/10 backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="mb-6 pt-2">
                <div className="flex rounded-2xl bg-slate-100/80 p-1.5 gap-1">
                    {variants.map((v, i) => {
                        const isPopular = variants.length === 3 && i === 1;
                        return (
                            <button
                                key={v.name}
                            ...
                            >
                                {/* Popular badge for middle variant */}
                                {isPopular && (
                                ...
                                <div className={cn(
                                    ...
                                    selectedIdx === i ? "text-slate-900" : "text-inherit"
                                )}>
                                    {v.name}
                                </div>
                                <div className={cn(
                                    "text-[11px] mt-0.5 transition-colors font-medium",
                                    selectedIdx === i ? "text-slate-500" : "text-slate-400"
                                )}>
                                    {v.price} PLN
                                </div>
                            </button>
            ...
            <div className="mb-6 text-center">
                <div className="text-sm text-slate-500 font-medium mb-1 uppercase tracking-wide">
                    {selected.label}
                </div>
                <div className="flex items-baseline justify-center gap-2">
                    <span className="text-5xl font-extrabold text-slate-900 tracking-tight transition-all duration-300">
                        {selected.price}
                    </span>
                    <span className="text-xl font-medium text-slate-400">PLN</span>
                </div>
                {selected.scope && (
                    <div className="mt-2 text-sm text-slate-500">{selected.scope}</div>
                )}
            ...
            <Button asChild className={`w-full h-auto py-4 text-lg bg-gradient-to-r ${gradient} hover:shadow-xl hover:shadow-indigo-500/30 hover:scale-[1.02] transition-all duration-300 rounded-xl`}>
                <Link href={`/app/company/packages/${packageId}/customize?variant=${selected.name}`}>
