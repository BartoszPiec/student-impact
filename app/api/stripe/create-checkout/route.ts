...
import { getStripe, calculatePlatformFeeWithRate, COMMISSION_RATE_USER } from "@/lib/stripe";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: contract, error: contractError } = await supabase
      .from("contracts")
      .select("id, company_id, student_id, total_amount, status, terms_status, commission_rate, milestones(id, title, amount, status)")
      .eq("id", contractId)
      .single();
    ...
    const commissionRate = (contract as any).commission_rate ?? COMMISSION_RATE_USER;
    const platformFee = calculatePlatformFeeWithRate(amountInGrosze, commissionRate);
