...
BEGIN
  ...
  INSERT INTO public.financial_ledger (
    ...
    p_payment_intent_id, p_session_id, 'WpĹ‚ata Stripe â€” kontrakt ' || p_contract_id
...
BEGIN
  ...
  INSERT INTO public.financial_ledger (
    ...
    'Zwrot Stripe (' || p_refund_id || ') â€” kwota ' || p_refund_amount_delta_pln || ' ' || p_currency,
...
BEGIN
  ...
  IF p_admin_id IS NULL THEN
    RAISE EXCEPTION 'Unauthorized: missing admin_id';
  END IF;
  ...
  VALUES ('student_payout', v_payout.amount_net, v_payout.amount_net_minor, 'PLN', 'debit', v_payout.contract_id, 'WypĹ‚ata Ĺ›rodkĂłw dla studenta (Payout ID: ' || p_payout_id || ')', jsonb_build_object('payout_id', p_payout_id, 'admin_id', p_admin_id))
  ON CONFLICT (id) DO NOTHING;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.process_stripe_payment_v4(text, text, uuid, uuid, uuid, numeric, numeric, uuid[], uuid, bigint, bigint) FROM anon;
REVOKE EXECUTE ON FUNCTION public.process_stripe_payment_v4(text, text, uuid, uuid, uuid, numeric, numeric, uuid[], uuid, bigint, bigint) FROM authenticated;
...
REVOKE EXECUTE ON FUNCTION public.process_stripe_refund_v4(text, text, text, numeric, text, text, bigint) FROM anon;
REVOKE EXECUTE ON FUNCTION public.process_stripe_refund_v4(text, text, text, numeric, text, text, bigint) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.process_stripe_refund_v4(text, text, text, numeric, text, text, bigint) TO service_role;
