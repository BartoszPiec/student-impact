-- FINAL ATTEMPT to Clear Ambiguity
-- We use DROP FUNCTION with CASCADE to ensure we really get rid of them

DROP FUNCTION IF EXISTS public.contract_upsert_milestone_draft(uuid, uuid, text, numeric, timestamp with time zone, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.contract_upsert_milestone_draft(uuid, uuid, text, numeric, timestamp with time zone, text) CASCADE;
DROP FUNCTION IF EXISTS public.contract_upsert_milestone_draft(uuid, uuid, text, numeric, timestamptz, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.contract_upsert_milestone_draft(uuid, uuid, text, numeric, timestamptz, text) CASCADE;
...
BEGIN
  ...
  -- Bump version logic (if status is proposed/agreed, hitting upsert creates a new draft version)
  ...
  ELSE
    ...
    SET title = p_title,
        ...
        terms_version = v_new_version,
        updated_by = auth.uid(), -- Mark as updated by current user
        updated_at = now()
