...
-- Fix draft_approve: set contract status to 'awaiting_funding' after approval
-- Previously it only set terms_status='agreed' but left status='draft'
...
BEGIN
    ...
    INSERT INTO milestones (
        contract_id, title, amount, acceptance_criteria, idx,
...
-- Also fix the current contract that's stuck in 'draft' with terms_status='agreed'
UPDATE contracts
SET status = 'awaiting_funding'
WHERE status = 'draft'
AND terms_status = 'agreed'
AND EXISTS (SELECT 1 FROM milestones m WHERE m.contract_id = contracts.id);
;
